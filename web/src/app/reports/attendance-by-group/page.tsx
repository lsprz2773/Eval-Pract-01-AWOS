...
export default function AttendanceByGroupPage() {
  ...
  const loadData = async (p: number) => {
    setLoading(true);
    setError('');
    const result = await getAttendanceByGroup({ page: p, limit });
    if (result.success) {
      setData(result.data ?? []);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900 py-12 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <h1 className="text-3xl font-bold text-black dark:text-white mb-8">Asistencia por Grupo</h1>
        <h2 className="text-3l font-bold text-black dark:text-white mb-8">Grupos con baja asistencia para seguimiento de deserción temprana.</h2>
                ...
                <tbody>
                  {data.map((row, idx) => (
                    <tr key={idx} className="border-t dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700">
                      ...
                      <td className="px-6 py-3 text-gray-700 dark:text-gray-300">{row.profesor}</td>
